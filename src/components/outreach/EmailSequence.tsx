...
export function EmailSequence({ record, generatingMail, onGenerateMail, onGenerateAll, onEditEmail }: EmailSequenceProps) {
  ...
  const TABS = [
    { n: 1 as const, label: "Accroche", timing: "J0" },
    { n: 2 as const, label: "ROI", timing: `J+${gap1}` },
    { n: 3 as const, label: "Closing", timing: `J+${gap1 + gap2}` },
  ];
          ...
          <div className="animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {showCustomPrompt ? "Fermer prompt" : "Prompt custom"}
                </button>
                <button
                ...
                <button
                  ...
                  title={sendStatus === "error" ? sendError : `Envoyer à ${defaultRecipient}`}
                >
            ...
            {showCustomPrompt && (
              <div className="mb-4 rounded-xl p-3" style={{ background: "#FFF0F5", border: "1px solid #F22E75" }}>
                <label className="text-[11px] font-bold uppercase tracking-[0.15em]" style={{ color: "#F22E75" }}>
                  Instructions custom (prioritaires sur le prompt par defaut)
                </label>
                <textarea
                  value={customPrompt}
                  onChange={(e) => setCustomPrompt(e.target.value)}
                  placeholder="Ex: plus court, ton plus direct, insiste sur le Black Friday, mentionne que j'ai vu leur post LinkedIn..."
                  rows={3}
                ...
                <div className="mt-2 flex justify-end">
                  ...
                  >
                    {generatingMail === activeTab ? "Regeneration..." : "Regenerer avec ces instructions"}
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="text-center py-12">
            {generatingMail === activeTab ? (
              <div className="animate-fade-in">
                ...
                <p className="text-[14px] font-bold" style={{ color: "#03182F" }}>Génération du mail {activeTab} en cours...</p>
                <p className="text-[12px] mt-2" style={{ color: "#6B7280" }}>Analyse concurrentielle + calcul ROI personnalisé...</p>
              </div>
            ) : (
              <div>
                <p className="text-[14px] mb-4" style={{ color: "#6B7280" }}>Mail non encore généré</p>
                <button
                  onClick={() => onGenerateMail(activeTab)}
